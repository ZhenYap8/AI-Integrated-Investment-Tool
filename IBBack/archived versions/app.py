...
def normalize_period(s: Optional[str]) -> Optional[str]:
    ...
    if val == "all":
        val = "max"
    # accept 1Y/3M forms
    if val.endswith("y") and val[:-1].isdigit():
        pass
    elif val.endswith("mo") and val[:-2].isdigit():
        pass
    elif val.endswith("d") and val[:-1].isdigit():
        pass
    if val in ALLOWED_PERIODS:
        return val
    return None
...
def analyze_company(ticker: str, years: Optional[int] = None, period: Optional[str] = None) -> AnalyzeResponse:
    t = get_yf_ticker(ticker)

    # Price & info
    norm_period = normalize_period(period)
    hist = t.history(period=norm_period or "max")
    if years and not norm_period:
...
def analyze_industry(industry: str, years: Optional[int] = None, period: Optional[str] = None) -> AnalyzeResponse:
    ...
    comps = [analyze_company(t, years, period=period) for t in tickers]
...
async def analyze(
    ...
    norm_period = normalize_period(range_ or period)
    ...
    if is_company:
        ...
        for c in DEMO_COMPANIES:
            if val.upper() == c["value"] or val.lower() == c["label"].lower():
                return analyze_company(c["value"], years, period=norm_period)
        # fallback: treat as ticker
        return analyze_company(val.upper(), years, period=norm_period)

    if is_industry:
        # pick canonical industry key
        for i in DEMO_INDUSTRIES:
            if val.lower() == i["value"].lower() or val.lower() == i["label"].lower():
                return analyze_industry(i["value"], years, period=norm_period)
        return analyze_industry(val, years, period=norm_period)
    ...
    return analyze_industry("Robotics", years, period=norm_period)
